...
namespace INIFILE
{
    public abstract class CustomIniFile
    {
        ...
        public virtual bool SectionExits(string Section)
        {
    ...
    public class IniFile : CustomIniFile
    {
        ...
        private static extern uint GetPrivateProfileString(
            ...
            string lpDefaule,   // points to default string
        ...
        private static extern bool WritePrivateProfileString(
            string lpAppName,   // points to section name
            string lpKeyName,   // points to key name
            string lpString,   // points to string to add
            string lpFileName   // points to initialization filename
         );
        ...
        public IniFile(string AFileName):base(AFileName) {}
        ...
        public override bool ReadSection(string Section, List<string> Strings)
        {
            ...
            int j = 0;
            uint vLength = GetPrivateProfileString(Section, null, null,
                vBuffer, (uint)vBuffer.Length, FileName);
        ...
        public override bool ReadSections(List<string> Strings)
        {
            byte[] vBuffer = new byte[16384];
            int j = 0;
            uint vLength = GetPrivateProfileString(null, null, null,
                vBuffer, (uint)vBuffer.Length, FileName);
            for (int i = 0; i < vLength; i++)
            {
                if (vBuffer[i] == 0)
        ...
        public override string ReadString(string Section, string Ident, string Default)
        {
            ...
            uint vCount = GetPrivateProfileString(Section, Ident, Default, 
                vBuffer, (uint)vBuffer.Length, FileName);
